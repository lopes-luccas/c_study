...
class JogoAdvinhacao {
	...
	public:	
		...
		void imprimirJogo() {
			...
			cout << "\t\t| Selecione um nível de dificuldade: ";
		}
		...
		void verificarDificuldade(int dificuldade) {
			switch(dificuldade) {
				case 1: 
					...
					cout << "\t\t|       Nível fácil selecionado       |\n";
					break;
				case 2:
					tentativas = 10;
					dificuldadeValida = true;
					cout << "\t\t|       Nível médio selecionado       |\n";
					break;
				case 3:
					tentativas = 5;
					dificuldadeValida = true;
					cout << "\t\t|      Nível difícil selecionado      |\n";
					break;
				default:
					cout << "\t\t|    Nível de dificuldade inválida!   |\n\t\t|  Seleciona uma dificuldade válida: ";
			}
		}
		bool dificuldadeValidaSelecionada() {
//			Configurar Variável de instância 'dificuldadeValida' para falso.
		...
		void mostrarTentativasRestantes() { 
			cout << "\t\t|                                     |\n";
			if(tentativas < 10) {
				cout << "\t\t|    Você tem [" << tentativas << "] chutes restando     |\n";		
			} else {
				cout << "\t\t|    Você tem [" << tentativas << "] chutes restando    |\n";	
			}
		}
		...
		void perguntarSeQuerJogarDeNovo() {
			...
			while(true) {
				...
				if(tentarNovamente == 's' || tentarNovamente == 'S') {
					...
					cout << "\t\t|Não foi possível entender sua reposta|\n";
				}	
			}
		}
		...
		void parabenizar() {
			...
			if(pontuacao < 10) {
				cout << "\t\t|       Você obteve " << pontuacao << " pontos!         |" << endl;	
		...
		void amenizarDerrota() {
			cout << "\t\t|                                     |\n";
			cout << "\t\t|  Ahh, que pena, não foi dessa vez!  |\n";
			perguntarSeQuerJogarDeNovo();
			jogarNovamente();
		}
		void regularChute() {
			...
			if(chute > numeroSecreto) {
				cout << "\t\t|    Você errou! É um número menor!   |\n";
			} else {
				cout << "\t\t|    Você errou! É um número maior!   |\n";
			}
		}
		void chutar() {
			while(tentativas > 0) {
